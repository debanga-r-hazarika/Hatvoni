...
export default function TrackShipment() {
    ...
    return (
      <main className="pt-28 pb-16 md:pt-36 md:pb-20 min-h-screen bg-gradient-to-b from-background via-surface to-surface-container-low/80">
        <div className="max-w-lg mx-auto px-4 text-center">
          ...
          <h1 className="font-headline text-xl font-bold text-primary">Invalid tracking link</h1>
          ...
          <div className="mt-8 flex flex-col sm:flex-row gap-3 justify-center">
            ...
            <Link
              to="/"
              className="inline-flex items-center justify-center gap-2 rounded-xl border border-outline-variant/40 bg-surface px-5 py-3 text-sm font-bold text-primary font-headline hover:bg-surface-container-low transition-colors"
            >
              Home
            </Link>
          </div>
  ...
  return (
    <main className="pt-24 pb-12 md:pt-32 md:pb-16 min-h-screen bg-gradient-to-b from-background via-surface to-surface-container-low/90">
      <div className="max-w-6xl mx-auto px-4 sm:px-6">
        {/* Header */}
        <header className="mb-6 flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
          <div>
            <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-secondary mb-2">Shipment tracking</p>
            <h1 className="font-headline text-2xl md:text-3xl font-bold text-primary tracking-tight">
              Track your delivery
            </h1>
            <p className="text-sm text-on-surface-variant font-body mt-2 max-w-xl">
          ...
          <div className="flex flex-wrap items-center gap-2">
            <a
              ...
              className="inline-flex items-center gap-1.5 rounded-xl border border-outline-variant/35 bg-white px-4 py-2.5 text-xs font-bold text-primary shadow-sm hover:bg-surface-container-low transition-colors"
            >
            ...
            <Link
              to="/orders"
              className="inline-flex items-center gap-1.5 rounded-xl px-4 py-2.5 text-xs font-bold text-on-surface-variant hover:text-primary transition-colors"
            >
        ...
        {/* Embed card */}
        <div className="rounded-[1.75rem] border border-outline-variant/25 bg-white shadow-[0_20px_60px_rgba(0,74,43,0.07)] overflow-hidden">
          <div className="flex flex-wrap items-center justify-between gap-3 px-4 sm:px-6 py-3 border-b border-outline-variant/15 bg-surface-container-low/50">
            ...
            <p className="text-[10px] text-on-surface-variant/70 max-w-md text-right">
          ...
          <div className="relative bg-surface-container-low/40 min-h-[62vh] md:min-h-[68vh]">
            {!iframeLoaded && (
              <div className="absolute inset-0 flex flex-col items-center justify-center gap-3 z-10 bg-surface-container-low/60 backdrop-blur-[2px]">
                ...
                <p className="text-xs font-semibold text-on-surface-variant font-body">Loading tracking…</p>
              </div>
            )}
            <iframe
          ...
          <footer className="px-4 sm:px-6 py-3 flex flex-wrap items-center justify-between gap-2 border-t border-outline-variant/15 bg-white/90 text-[10px] text-on-surface-variant font-body">
            <span>
              Tracking hosted by{' '}
              <a
